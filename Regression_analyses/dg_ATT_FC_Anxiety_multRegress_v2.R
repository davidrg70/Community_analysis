...
model_a <- lm(High.Load.sgacc.and.l.ains ~ MASC2_Tscores + Sex, data = att_data1)
model_a_std <- lm.beta(model_a)
summary(model_a_std)
# sgacc and l a ins_BothLowLoad & covariates
model_b <- lm(Low.Load.sgacc.and.l.ains ~ MASC2_Tscores + Sex, data = att_data1)
model_b_std <- lm.beta(model_b)
summary(model_b_std)

# sgacc and l a ins_HighLoadFearAndMadFaces & covariates
model_c <- lm(High.Load.Fear.And.Mad.Faces.sgacc.and.l.ains ~ MASC2_Tscores + Sex, data = att_data2)
model_c_std <- lm.beta(model_c)
summary(model_c_std)
# sgacc and l a ins_LowLoadFearAndMadFaces & covariates
model_d <- lm(Low.Load.Fear.And.Mad.Faces.sgacc.and.l.ains ~ MASC2_Tscores + Sex, data = att_data2)
model_d_std <- lm.beta(model_d)
summary(model_d_std)

# l racc and r dacc_HighLoadFearAndMadFaces & covariates
model_e <- lm(High.Load.Fear.And.Mad.Faces.l.racc.and.r.dacc ~ MASC2_Tscores + Sex, data = att_data3)
model_e_std <- lm.beta(model_e)
summary(model_e_std)
# l racc and r dacc_LowLoadFearAndMadFaces & covariates
model_f <- lm(Low.Load.Fear.And.Mad.Faces.l.racc.and.r.dacc ~ MASC2_Tscores + Sex, data = att_data3)
model_f_std <- lm.beta(model_f)
summary(model_f_std)

# l racc and r dacc_HighLoadNeutralFaces & covariates
model_g <- lm(High.Load.Neutral.Faces.l.racc.and.r.dacc ~ MASC2_Tscores + Sex, data = att_data4)
model_g_std <- lm.beta(model_g)
summary(model_g_std)
# l racc and r dacc_LowLoadNeutralFaces & covariates
model_h <- lm(Low.Load.Neutral.Faces.l.racc.and.r.dacc ~ MASC2_Tscores + Sex, data = att_data4)
...
ggplot(att_data1, aes(x = High.Load.sgacc.and.l.ains, y = MASC2_Tscores, color = Sex, shape = Sex)) +
  geom_point(size = 4.5, alpha = 0.86) +
  geom_smooth(method = "lm", se = TRUE, linetype = "longdash", size = 0.7, alpha = 0.125) +
  labs(
    title = "Both HL conditions (SGACC & Left AIns)",
...
att_data1$Sex <- factor(att_data1$Sex, levels = c(0, 1), labels = c("Male", "Female"))
# adding a regression line per group
ggplot(att_data1, aes(x = Low.Load.sgacc.and.l.ains, y = MASC2_Tscores, color = Sex, shape = Sex)) +
  geom_point(size = 4.5, alpha = 0.86) +
  geom_smooth(method = "lm", se = TRUE, linetype = "longdash", size = 0.7, alpha = 0.125) +
  labs(
    title = "Both LL conditions (SGACC & Left AIns)",
    x = "Fisher z-transformed rho",
    y = "MASC2 T-scores"
  ) +
  scale_color_manual(values = c("royalblue", "darkorange")) +
...
# adding a regression line per group
ggplot(att_data4, aes(x = High.Load.Neutral.Faces.l.racc.and.r.dacc, y = MASC2_Tscores, color = Sex, shape = Sex)) +
  geom_point(size = 4.5, alpha = 0.86) +
  geom_smooth(method = "lm", se = TRUE, linetype = "longdash", size = 0.7, alpha = 0.125) +
  labs(
    title = "HL Neutral Faces (Left RACC & Right DACC)",
    x = "Fisher z-transformed rho",
    y = "MASC2 T-scores"
  ) +
  scale_color_manual(values = c("royalblue", "darkorange")) +
...
ggplot(att_data4, aes(x = Low.Load.Neutral.Faces.l.racc.and.r.dacc, y = MASC2_Tscores, color = Sex, shape = Sex)) +
  ...
  geom_smooth(method = "lm", se = TRUE, linetype = "longdash", size = 0.7, alpha = 0.125) +
  labs(
    title = "LL Neutral Faces (Left RACC & Right DACC)",
    x = "Fisher z-transformed rho",
    y = "MASC2 T-scores"
  ) +
  scale_color_manual(values = c("royalblue", "darkorange")) +
